...
tmp_long, tmp_base, tmp_mask, e_tmp, t_tmp, obs_time_1 = get_tensors(tmp_data.copy())
...
np.savetxt("surv_pred.csv", surv_pred, delimiter = ",")
...
event_tmp.to_csv("event_tmp.csv", index=False)
time_tmp = pd.DataFrame(t_tmp.numpy())
time_tmp.to_csv("time_tmp.csv", index=False)

event_train = pd.DataFrame(e_train.numpy().astype(int))
event_train.to_csv("event_train.csv", index=False)
